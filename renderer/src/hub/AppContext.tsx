...
const AppProvider: React.FC<React.PropsWithChildren> = ({ children }) => {
  ...
  const refreshAppList = useCallback(() => {
    return mds().then(async (response) => {
      ...
      await Promise.all(
        apps
          .filter((a: any) => !a.conf.system && a.conf.icon)
          .map(async (a: any) => {
            try {
              const url = `${mdsHost}${a.uid}/${encodeURI(a.conf.icon)}?uid=${a.sessionid}&v=${encodeURIComponent(a.conf.version || '')}`;
              const data = await (window as any).minima.iconData(url);
              if (data) {
                a.conf.overrideIcon = data;
                setIconFor(a.uid, data);
              }
            } catch (e) {
              /* fall back to the direct MDS url */
            }
          })
      );
