...
import { filePathOf, iconFor, update } from '../../../lib';
  ...
  const handleOnSubmit = async (evt: React.FormEvent) => {
    ...
    if (file) {
      try {
        ...
        // minimaDesk: update straight from the picked file's absolute path
        const installedInfo = await update(showUpdateApp.uid, filePathOf(file));
                        ...
                        {installed && (
                          <div className="text-center">
                            ...
                            <div
                              ...
                              style={{
                                backgroundImage: `url(${iconFor(installed.uid) || `${(window as any).MDS.filehost}${installed.uid}/${installed.conf.icon}?q=${Date.now()}`})`,
                              }}
                            />
                            <button
                              onClick={onClose}
