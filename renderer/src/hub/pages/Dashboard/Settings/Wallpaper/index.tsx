...
import { filePathOf, set } from '../../../../lib';
...
export function Wallpaper({ display, dismiss }: WallpaperProps) {
  ...
  const setCustomWallpaper = async () => {
    ...
    if (file) {
      ...
      // store wallpaper file name in background key / value item
      await set('BACKGROUND', 'custom-' + res.fileName);

      document.body.style.backgroundImage = `url("${res.dataUrl}")`;
      ...
      setActiveWallpaper('custom-' + res.fileName);
    }
  };
                ...
                <div className="bg-contrast1 p-4 rounded">
                  ...
                  <div className={`mb-4 ${showCustomWallpaperBlock ? 'block' : 'hidden'}`}>
                    ...
                    <div
                      ...
                      style={{ backgroundImage: customWallpaper ? `url("${customWallpaper}")` : undefined }}
                    />
                  </div>
