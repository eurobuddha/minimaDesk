...
const TitleBar = () => {
  ...
  const displayBlockInfo = async (evt) => {
    ...
    if (isNodeFiveMinutesAgoBehind && securityApp) {
      ...
      return shell.openDapp({ uid: link.uid, sessionid: link.sessionid, name: securityApp.conf.name, hash: '#/dashboard/quicksync/host' });
    }
    ...
    if (statusApp) {
      ...
      return shell.openDapp({ uid: link.uid, sessionid: link.sessionid, name: statusApp.conf.name });
    }
    ...
    if (fallbackHealthApp) {
      ...
      return shell.openDapp({ uid: link.uid, sessionid: link.sessionid, name: fallbackHealthApp.conf.name });
    }
      ...
      <div className="grid grid-cols-12 h-full">
        ...
        <div className="svg col-span-9 flex items-center justify-end">
          <div className="absolute top-0 right-3 flex p-4 pr-0 items-center justify-end gap-2 overflow-hidden">
            ...
            {/* minimaDesk: no MDS session to log out of — the stock 'Log out' form is removed */}
          </div>
        </div>
      </div>
    </div>
