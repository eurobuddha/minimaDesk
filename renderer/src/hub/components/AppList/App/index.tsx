...
const AppList = ({ data }) => {
  ...
  const openApp = async () => {
    ...
    // minimaDesk: open the dapp as a tab in the shell (stock hub: window.open of the MDS url)
    shell.openDapp({ uid: data.uid, sessionid: data.sessionid, name: data.conf.name, icon: data.conf.overrideIcon });
  };
    ...
    <div className="app-grid__icon mb-4 sm:mb-6 lg:mb-8">
      <div className="relative">
        ...
        <div
          ...
          className={`${['Dapp Store', 'minimaCore App Store'].includes(data.conf.name) ? 'dapp_store' : ''} ${
